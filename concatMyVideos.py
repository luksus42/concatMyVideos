...
class Process:
  ...
  def trim(self, start, length, filePath, output):
    ...
    ### SCALE ###
    commandScale = ["ffmpeg","-i",tempFile,"-vf","scale=iw*min(1920/iw\,1080/ih):ih*min(1920/iw\,1080/ih), pad=1920:1080:(1920-iw*min(1920/iw\,1080/ih))/2:(1080-ih*min(1920/iw\,1080/ih))/2",
    "-c:v","h264_nvenc","-preset","llhq","-rc:v","vbr_minqp","-qmin:v","20","-qmax:v","23","-b:v","3000k","-maxrate:v","6000k","-profile:v","high","-r","30","-c:a","copy","-y",tempScaleFile]
    if not self.verbose:
      commandScale.extend(suppressVerbose)
    subprocess.call(commandScale)  

    ### ADD OVERLAY ###
    commandOverlay = ["ffmpeg","-i",tempScaleFile,"-vf","drawtext='text="+filename[:-6]+": fontcolor=white: fontsize=32: box=1: boxcolor=black@0.5: boxborderw=5: x=(w-text_w-50): y=(h-text_h-40)'"]
    commandOverlay2 = ["-c:v","h264_nvenc","-preset","llhq","-rc:v","vbr_minqp","-qmin:v","20","-qmax:v","23","-b:v","3000k","-maxrate:v","6000k","-profile:v","high","-r","30","-c:a","copy",output]
    commandOverlay.extend(commandOverlay2)
    if not self.verbose:
      commandOverlay.extend(suppressVerbose)
    subprocess.call(commandOverlay)
